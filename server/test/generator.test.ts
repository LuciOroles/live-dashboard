...
describe('paramter generator', () => {
    test('should get a value each time it is called', () => {
        ...
        for (let i = 0; i < 10; i++) {
            expect(Number.isFinite(generator.next({ second: i }).value)).toBeTruthy();
        }
    });
    ...
    test('sending stop will force the generator to return 0', () => {
        ...
        const v1 = generator.next({ second: 1 }).value;
        expect(v1).toBeGreaterThan(0);

        const v2 = generator.next({ second: 2, stop: true }).value;
        expect(v2).toEqual(0);

        const v3 = generator.next({ second: 99, stop: false }).value;
